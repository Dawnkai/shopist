...
export default function AddUnitModal({
  ...
  const newUnit = useSelector((state: RootState) => state.unit.selectedUnit);
  const dispatch = useDispatch();

  const handleClose = () => setVisible(false);

  const formValid = () => {
    if (newUnit.unitDisplayName === '') return false;
    if (newUnit.unitNum <= 0) return false;
    return true;
  };
    ...
    <Modal show={visible} onHide={handleClose}>
      ...
      <Modal.Footer>
        ...
        <Button
          variant="success"
          onClick={() => {
            if (formValid()) handleSubmit(newUnit);
          }}
        >
          Add
        </Button>
